...
class DashboardController extends Controller
{
    ...
    public function statsKependudukan(Request $request)
    {
        ...
        return Cache::remember($cacheKey, self::CACHE_TTL, function () use ($desa) {
            $baseQuery = DB::table('t_kartu_keluarga_anggota as t1')
    ...
    public function statsKepemilikanRumah(Request $request)
    {
        ...
        return Cache::remember($cacheKey, self::CACHE_TTL, function () use ($desa) {
            $stats = DB::table('t_kartu_keluarga_anggota as t1')
                ->join('t_kartu_keluarga as t2', 't1.no_kk', '=', 't2.id')
                ->select(
                    DB::raw('COALESCE(t1.kepemilikan_rumah, "Tidak Diketahui") as kepemilikan_rumah'),
                    DB::raw('COUNT(*) as jumlah')
                )
                ->when($desa !== 'all', fn($q) => $q->where('t2.desa', $desa))
    ...
    public function statsPendapatan(Request $request)
    {
        ...
        return Cache::remember($cacheKey, self::CACHE_TTL, function () use ($desa) {
            $stats = DB::table('t_kartu_keluarga_anggota as t1')
                ->join('t_kartu_keluarga as t2', 't1.no_kk', '=', 't2.id')
                ->select(
                    DB::raw('COALESCE(t1.pendapatan_perbulan, "Tidak Diketahui") as pendapatan'),
                    DB::raw('COUNT(*) as jumlah')
                )
                ->when($desa !== 'all', fn($q) => $q->where('t2.desa', $desa))
                ->groupBy('pendapatan')
    ...
    public function statsPerkawinan(Request $request)
    {
        ...
        return Cache::remember($cacheKey, self::CACHE_TTL, function () use ($desa) {
            ...
            $kawinTercatat = $stats->where('sts_perkawinan', 'KAWIN')
                ->whereNotNull('status_kawin_tercatat')
                ->sum('jumlah');
            ...
            return response()->json([
                ...
                'chart_data' => [
                    ['label' => 'Kawin', 'value' => (int) $kawin],
                    ['label' => 'Belum Kawin', 'value' => (int) $belumKawin]
                ],
            ]);
        });
    }
    ...
    public function clearCache()
    {
        ...
        foreach ($patterns as $pattern) {
            if (str_contains($pattern, '*')) {
                // For wildcard patterns, you may need to use Cache::tags() or manually track keys
